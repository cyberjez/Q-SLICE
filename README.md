...
Q-SLICE v3 introduces user-driven configuration. Researchers can input custom values for shots, Shor’s N, Bell error rate and RNG bias. This enables flexible scenario modelling and comparative testing across environments. The harness includes layered fallbacks (Aer to BasicAer to Statevector) and simulated noise injection, ensuring robustness even when full quantum backends are unavailable. qslice_gui incorperates a GUI and a report format output which can be easily copied into a reports or analysed for further tests. qlsice_dashboard adds graphs and metrics explanation. Different versions for different use case and preferences.
...
Instructions
Install and run Miniconda: https://www.anaconda.com/download. Create an environment: conda create --name quantum and activate environment: conda activate quantum. Install PIP to install packages: conda install PIP, install QISKIT: Pip install Qiskit. Type in cd .conda. Download QSLICE Threat Harness and put into .conda directory which is usually in the  windows users directory.
